...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			{
				CrosshairInAirFactor = FMath::FInterpTo(CrosshairInAirFactor, 1.f, DeltaTime, 1.f);
			}
			else
			{
			...
			if (bAiming)
			{
				CrosshairAimFactor = FMath::FInterpTo(CrosshairAimFactor, 0.4f, DeltaTime, 20.f);
			}
			else
			{
			...
			{
				CrosshairCrouchFactor = FMath::FInterpTo(CrosshairCrouchFactor, 0.3f, DeltaTime, 20.f);
			}
			else
			{
				CrosshairCrouchFactor = FMath::FInterpTo(CrosshairCrouchFactor, 0.f, DeltaTime, 20.f);
			}
...
void UCombatComponent::UpdateCarriedAmmo()
{
	...
	if (Controller)
	{
		...
		// Optional challenge weapon type
...
void UCombatComponent::SetSprinting(bool bIsSprinting)
{
	...
	if (Character->IsLocallyControlled()) bSprintButtonPressed = bIsSprinting;
}
...
void UCombatComponent::OnRep_Sprinting()
{
	if (Character && Character->IsLocallyControlled())
	{
		bSprinting = bSprintButtonPressed;
	}
}
...
{
	if (EquippedWeapon == nullptr) return;
	// Shotgun special case of firing while reloading individual shells
...
bool UCombatComponent::CanFire()
{
	...
	return !EquippedWeapon->IsEmpty() && bCanFire && CombatState == ECombatState::ECS_Unoccupied && !bSprinting;
}
...
void UCombatComponent::ThrowGrenade()
{
	...
	if (CombatState != ECombatState::ECS_Unoccupied || EquippedWeapon == nullptr || bSprinting) return;
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceHitResult)
{
	...
	if (bScreenToWorld)
	{
		...
		// Push trace past camera so character won't target self
		if (Character)
		{
			float DistanceToCharacter = (Character->GetActorLocation() - Start).Size();
			Start += CrosshairWorldDirection * (DistanceToCharacter + TraceCrosshairPushDistance);
		}
		...
		if (TraceHitResult.GetActor() && TraceHitResult.GetActor()->Implements<UInteractWithCrosshairsInterface>())
		{
			...
			CrosshairAimAtPlayerFactor = -0.1f;
			// Show enemy name
		}
		else
		{
